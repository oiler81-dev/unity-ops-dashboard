/**
 * Azure Function: ringcentral-spineone
 * Timer-triggered — runs every hour to pull RingCentral call logs for SpineOne (Denver).
 * Writes normalized data to Azure Table Storage (CallData table).
 *
 * Deploy to: /api/ringcentral-spineone/index.js
 *
 * Required App Settings:
 *   RC_SPINEONE_CLIENT_ID       — RingCentral app client ID
 *   RC_SPINEONE_CLIENT_SECRET   — RingCentral app client secret
 *   RC_SPINEONE_USERNAME        — RingCentral account phone/email
 *   RC_SPINEONE_PASSWORD        — RingCentral account password
 *   RC_SPINEONE_EXTENSION       — RingCentral extension (optional, omit for main account)
...
async function getRcToken() {
  const creds = Buffer.from(
    `${process.env.RC_SPINEONE_CLIENT_ID}:${process.env.RC_SPINEONE_CLIENT_SECRET}`
  ).toString("base64");

  const body = new URLSearchParams({
    grant_type: "password",
    username:   process.env.RC_SPINEONE_USERNAME,
    password:   process.env.RC_SPINEONE_PASSWORD,
    extension:  process.env.RC_SPINEONE_EXTENSION || ""
  }).toString();
...
module.exports = async function (context) {
  context.log("RingCentral SpineOne sync started");
  ...
  try {
    ...
    context.log(`RingCentral SpineOne: pulled ${records.length} call records`);

    const aggs  = aggregateByDate(records);
    const saved = await saveToTable(client, aggs);
    context.log("RingCentral SpineOne: saved", saved);

  } catch (error) {
    context.log.error("RingCentral SpineOne sync error:", error.message);
  }
};
