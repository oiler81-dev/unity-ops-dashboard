...
function renderDashboardCards(current, comparison, compareAgainst) {
  const cards = [
    ...
    },
    {
      ...
      meta: `${(current.totals?.visitVolume || 0) - (comparison.totals?.visitVolume || 0) >= 0 ? "+" : ""}${(current.totals?.visitVolume || 0) - (comparison.totals?.visitVolume || 0)} vs ${compareAgainst === "priorPeriod" ? "prior period" : compareAgainst}`
    },
    {
      label: "Call Volume",
      value: current.totals?.callVolume || 0,
      meta: `${(current.totals?.callVolume || 0) - (comparison.totals?.callVolume || 0) >= 0 ? "+" : ""}${(current.totals?.callVolume || 0) - (comparison.totals?.callVolume || 0)} vs ${compareAgainst === "priorPeriod" ? "prior period" : compareAgainst}`
    },
    {
      label: "New Patients",
      value: current.totals?.newPatients || 0,
      meta: `${(current.totals?.newPatients || 0) - (comparison.totals?.newPatients || 0) >= 0 ? "+" : ""}${(current.totals?.newPatients || 0) - (comparison.totals?.newPatients || 0)} vs ${compareAgainst === "priorPeriod" ? "prior period" : compareAgainst}`
    },
    {
      label: "Avg No Show %",
...
async function loadDashboardLanding() {
  ...
  const comparison = await loadDashboardDataForWeeks(weekSets.comparisonWeeks, entityScope);
    ...
    if (["budget", "target", "forecast"].includes(compareAgainst)) {
      noticeEl.innerHTML = `
        <div class="warning" style="padding:10px 12px; border:1px solid #1d435b; border-radius:8px; background:#0a2233;">
          ${compareAgainst.charAt(0).toUpperCase() + compareAgainst.slice(1)} comparison structure is in place, but the benchmark data layer is not wired into this restored front-end yet. Current view is showing actuals cleanly so the site stays stable.
        </div>
      `;
    } else {
      noticeEl.innerHTML = "";
